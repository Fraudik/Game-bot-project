...
class Fraudbot(discord.Client):
    def __init__(self, **options):
        ...
        games = '/быки и коровы (/бик) -- математическая игра, в двух изданиях: в одиночку и против бота. Две вариаци'\
                'и.\n/камень-ножницы-бумага (/кнб) -- классические камень-ножницы-бумага, с самообучающимся ботом.\n' \
                '/кости -- вы делаете ставки на сумму выброшенных ботом костей. Две вариации.\n\n' \
        ...
        self.commands = ['/помощь', '/игры', '/привет', '/бик', '/кнб', '/кости', '/быки и коровы',
    ...
    async def on_message(self, message):
        ...
        if message.content == '/быки и коровы' or message.content == '/бик':
        ...
        # запуск игры "Кости"
            ...
            await message.channel.send('Хорошо, ' + user_player + '! Выберите режим игры: простая игра в кости против'
                                                                  ...
                                                                  ' Внимание, второй вариант требует расчетов.\n\t1\t|'
                                                                  '\t2\t|\t/стоп')
            ...
            if variation.content == '/стоп' or self.user_status(user_player) == 'empty':
                ...
                while bot_cash != 0 and player_cash != 0:
                    ...
                    while True:
                        try:
                            if bet.content == '/стоп' or min(1, bot_cash) <= int(bet.content) <= min(20, player_cash)\
                                    or self.user_status(user_player) == 'empty':
                                break
                            raise ValueError
                        except ValueError:
                ...
                await message.channel.send(f'Вы сделали выбор, {user_player}. Правила таковы -- у вас ровно 100 монет. '
                                           ...
                                           f' вам всегда будет доступна монетка со стабильным коэффициентом 2.\n'
                                           ...
                                           f' все коэффициенты, кроме стабильного, варьируются от 2 до самих себя.\n'
        ...
        elif message.content == '/камень-ножницы-бумага' or message.content == '/кнб':
            # изменение статуса
